...
var ResourcePath = class _ResourcePath extends BasePath {
  ...
  static fromString(...t) {
    ...
    for (const r of t) {
      if (r.indexOf("//") >= 0)
        throw new FirestoreError(T, `Invalid segment (${r}). Paths must not contain // in them.`);
...
var FieldPath$1 = class _FieldPath$1 extends BasePath {
  ...
  static fromServerFormat(t) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (0 === r.length)
        throw new FirestoreError(T, `Invalid field path (${t}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      e.push(r), r = "";
    };
    let i = false;
    for (; n < t.length; ) {
      ...
      if ("\\" === e2) {
        if (n + 1 === t.length)
          throw new FirestoreError(T, "Path has trailing escape character: " + t);
        const e3 = t[n + 1];
        if ("\\" !== e3 && "." !== e3 && "`" !== e3)
          throw new FirestoreError(T, "Path has invalid escape sequence: " + t);
        r += e3, n += 2;
      } else
        "`" === e2 ? (i = !i, n++) : "." !== e2 || i ? (r += e2, n++) : (__PRIVATE_addCurrentSegment(), n++);
    }
    if (__PRIVATE_addCurrentSegment(), i)
      throw new FirestoreError(T, "Unterminated ` in path: " + t);
...
function __PRIVATE_validateNonEmptyArgument(t, e, r) {
  if (!r)
    throw new FirestoreError(T, `Function ${t}() cannot be called with an empty ${e}.`);
}
function __PRIVATE_validateDocumentPath(t) {
  if (!DocumentKey.isDocumentKey(t))
    throw new FirestoreError(T, `Invalid document reference. Document references must have an even number of segments, but ${t} has ${t.length}.`);
}
function __PRIVATE_validateCollectionPath(t) {
  if (DocumentKey.isDocumentKey(t))
    throw new FirestoreError(T, `Invalid collection reference. Collection references must have an odd number of segments, but ${t} has ${t.length}.`);
}
...
function __PRIVATE_cast(t, e) {
  ...
  (t = t._delegate), !(t instanceof e)) {
    if (e.name === t.constructor.name)
      throw new FirestoreError(T, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
...
function __PRIVATE_validatePositiveNumber(t, e) {
  if (e <= 0)
    throw new FirestoreError(T, `Function ${t}() requires a positive number, but it was: ${e}.`);
}
...
function __PRIVATE_mapCodeFromHttpStatus(t) {
  if (void 0 === t)
    return __PRIVATE_logError("RPC_ERROR", "HTTP error has no status"), A;
  switch (t) {
    case 200:
      return E;
    case 400:
...
function __PRIVATE_randomBytes(t) {
  ...
  if (e && "function" == typeof e.getRandomValues)
    ...
    for (let e2 = 0; e2 < t; e2++)
      r[e2] = Math.floor(256 * Math.random());
  return r;
}
...
var Timestamp = class _Timestamp {
  ...
  constructor(t, e) {
    if (this.seconds = t, this.nanoseconds = e, e < 0)
      throw new FirestoreError(T, "Timestamp nanoseconds out of range: " + e);
    if (e >= 1e9)
      throw new FirestoreError(T, "Timestamp nanoseconds out of range: " + e);
    if (t < -62135596800)
      throw new FirestoreError(T, "Timestamp seconds out of range: " + t);
    if (t >= 253402300800)
      throw new FirestoreError(T, "Timestamp seconds out of range: " + t);
  }
  /**
  ...
  switch (r) {
    ...
    case 2:
      return function __PRIVATE_numberEquals(t2, e2) {
        if ("integerValue" in t2 && "integerValue" in e2)
          return __PRIVATE_normalizeNumber(t2.integerValue) === __PRIVATE_normalizeNumber(e2.integerValue);
    ...
    case 10:
      return function __PRIVATE_objectEquals(t2, e2) {
        ...
        if (__PRIVATE_objectSize(r2) !== __PRIVATE_objectSize(n))
          return false;
        for (const t3 in r2)
          if (r2.hasOwnProperty(t3) && (void 0 === n[t3] || !__PRIVATE_valueEquals(r2[t3], n[t3])))
            return false;
        return true;
      }(t, e);
    default:
...
function __PRIVATE_valueCompare(t, e) {
  ...
  if (r !== n)
    return __PRIVATE_primitiveComparator(r, n);
  switch (r) {
    case 0:
    case 9007199254740991:
      return 0;
...
function __PRIVATE_compareTimestamps(t, e) {
  if ("string" == typeof t && "string" == typeof e && t.length === e.length)
    return __PRIVATE_primitiveComparator(t, e);
...
function __PRIVATE_deepClone(t) {
  ...
  if (t.arrayValue) {
    ...
    for (let r = 0; r < (t.arrayValue.values || []).length; ++r)
      e.arrayValue.values[r] = __PRIVATE_deepClone(t.arrayValue.values[r]);
    return e;
  }
...
function __PRIVATE_boundEquals(t, e) {
  ...
  for (let r = 0; r < t.position.length; r++) {
    if (!__PRIVATE_valueEquals(t.position[r], e.position[r]))
      return false;
  }
  return true;
}
...
var SortedMapIterator = class {
  constructor(t, e, r, n) {
    ...
    let i = 1;
    for (; !t.isEmpty(); )
      if (i = e ? r(t.key, e) : 1, // flip the comparison if we're going in reverse
      e && n && (i *= -1), i < 0)
...
var LLRBNode = class _LLRBNode {
  ...
  remove(t, e) {
    let r, n = this;
    if (e(t, n.key) < 0)
      n.left.isEmpty() || n.left.isRed() || n.left.left.isRed() || (n = n.moveRedLeft()), n = n.copy(null, null, null, n.left.remove(t, e), null);
    else {
...
var SortedSet = class _SortedSet {
  ...
  forEachInRange(t, e) {
    ...
    for (; r.hasNext(); ) {
      const n = r.getNext();
      if (this.comparator(n.key, t[1]) >= 0)
        return;
      e(n.key);
    }
  }
  ...
  field(t) {
      ...
      return this.value;
    {
      ...
      for (let r = 0; r < t.length - 1; ++r)
        if (e = (e.mapValue.fields || {})[t.get(r)], !__PRIVATE_isMapValue(e))
          return null;
...
function __PRIVATE_queryNormalizedOrderBy(t) {
  ...
  if (null === e.S) {
    ...
    for (const r2 of e.explicitOrderBy)
      e.S.push(r2), t2.add(r2.field.canonicalString());
...
function __PRIVATE__queryToTarget(t, e) {
  if ("F" === t.limitType)
    return __PRIVATE_newTarget(t.path, t.collectionGroup, e, t.filters, t.limit, t.startAt, t.endAt);
  {
    e = e.map((t2) => {
...
function __PRIVATE_queryEquals(t, e) {
  return function __PRIVATE_targetEquals(t2, e2) {
    ...
    for (let r = 0; r < t2.orderBy.length; r++)
      if (!__PRIVATE_orderByEquals(t2.orderBy[r], e2.orderBy[r]))
        return false;
    if (t2.filters.length !== e2.filters.length)
      return false;
    for (let r = 0; r < t2.filters.length; r++)
      if (!__PRIVATE_filterEquals(t2.filters[r], e2.filters[r]))
        return false;
...
function fromName(t, e) {
  ...
  if (r.get(1) !== t.databaseId.projectId)
    throw new FirestoreError(T, "Tried to deserialize key from different project: " + r.get(1) + " vs " + t.databaseId.projectId);
  if (r.get(3) !== t.databaseId.database)
    throw new FirestoreError(T, "Tried to deserialize key from different database: " + r.get(3) + " vs " + t.databaseId.database);
...
function __PRIVATE_toFilter(t) {
  return t instanceof FieldFilter ? function __PRIVATE_toUnaryOrFieldFilter(t2) {
    if ("==" === t2.op) {
      if (__PRIVATE_isNanValue(t2.value))
        return {
          unaryFilter: {
      ...
      if (__PRIVATE_isNullValue(t2.value))
        return {
          unaryFilter: {
            field: __PRIVATE_toFieldPathReference(t2.field),
            op: "IS_NULL"
...
var __PRIVATE_DatastoreImpl = class extends class Datastore {
  ...
  Z() {
    if (this.Y)
      throw new FirestoreError(w, "The client has already been terminated.");
  }
...
async function __PRIVATE_invokeRunAggregationQueryRpc(t, e, r) {
  var n;
  const i = __PRIVATE_debugCast(t), { request: s, X: o, parent: a } = function __PRIVATE_toRunAggregationQueryRequest(t2, e2, r2) {
    const { B: n2, parent: i2 } = __PRIVATE_toQueryTarget(t2, e2), s2 = {}, o2 = [];
    let a2 = 0;
    return r2.forEach((t3) => {
      const e3 = "aggregate_" + a2++;
      s2[e3] = t3.alias, "count" === t3.aggregateType ? o2.push({
        alias: e3,
        count: {}
      }) : "avg" === t3.aggregateType ? o2.push({
        alias: e3,
        avg: {
          field: __PRIVATE_toFieldPathReference(t3.fieldPath)
        }
      }) : "sum" === t3.aggregateType && o2.push({
        alias: e3,
        sum: {
          field: __PRIVATE_toFieldPathReference(t3.fieldPath)
        }
...
var FirestoreSettingsImpl = class {
  constructor(t) {
    ...
    if (void 0 === t.host) {
      if (void 0 !== t.ssl)
        throw new FirestoreError(T, "Can't provide ssl option if host option is not set");
      this.host = "firestore.googleapis.com", this.ssl = true;
    } else
      this.host = t.host, this.ssl = null === (e = t.ssl) || void 0 === e || e;
    if (this.credentials = t.credentials, this.ignoreUndefinedProperties = !!t.ignoreUndefinedProperties, this.localCache = t.localCache, void 0 === t.cacheSizeBytes)
      this.cacheSizeBytes = 41943040;
    else {
      if (-1 !== t.cacheSizeBytes && t.cacheSizeBytes < 1048576)
        throw new FirestoreError(T, "cacheSizeBytes must be at least 1048576");
      this.cacheSizeBytes = t.cacheSizeBytes;
    }
    !function __PRIVATE_validateIsNotUsedTogether(t2, e2, r2, n) {
      if (true === e2 && true === n)
        throw new FirestoreError(T, `${t2} and ${r2} cannot be used together.`);
      ...
      if (void 0 !== t2.timeoutSeconds) {
        if (isNaN(t2.timeoutSeconds))
          throw new FirestoreError(T, `invalid long polling timeout: ${t2.timeoutSeconds} (must not be NaN)`);
        if (t2.timeoutSeconds < 5)
          throw new FirestoreError(T, `invalid long polling timeout: ${t2.timeoutSeconds} (minimum allowed value is 5)`);
        if (t2.timeoutSeconds > 30)
          throw new FirestoreError(T, `invalid long polling timeout: ${t2.timeoutSeconds} (maximum allowed value is 30)`);
      }
...
var Firestore = class {
  ...
  get app() {
    if (!this._app)
      throw new FirestoreError(w, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(t) {
    if (this._settingsFrozen)
      throw new FirestoreError(w, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
...
function initializeFirestore(t, e, r) {
  ...
  if (n.isInitialized(r))
    throw new FirestoreError(w, "Firestore can only be initialized once per app.");
  return n.initialize({
    options: e,
...
function connectFirestoreEmulator(t, e, r, n = {}) {
  ...
  if ("firestore.googleapis.com" !== s.host && s.host !== o && __PRIVATE_logWarn("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used."), t._setSettings(Object.assign(Object.assign({}, s), {
    ...
    if ("string" == typeof n.mockUserToken)
      e2 = n.mockUserToken, r2 = User.MOCK_USER;
    else {
      ...
      if (!s2)
        throw new FirestoreError(T, "mockUserToken must contain 'sub' or 'user_id' field!");
      r2 = new User(s2);
    }
...
function collection(t, e, ...r) {
  ...
  {
    if (!(t instanceof DocumentReference || t instanceof CollectionReference))
      throw new FirestoreError(T, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
...
function collectionGroup(t, e) {
  if (t = __PRIVATE_cast(t, Firestore), __PRIVATE_validateNonEmptyArgument("collectionGroup", "collection id", e), e.indexOf("/") >= 0)
    throw new FirestoreError(T, `Invalid collection ID '${e}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
  return new Query(
    t,
    /* converter= */
    null,
...
var FieldPath = class {
  ...
  constructor(...t) {
    for (let e = 0; e < t.length; ++e)
      if (0 === t[e].length)
        throw new FirestoreError(T, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
var GeoPoint = class {
  ...
  constructor(t, e) {
    if (!isFinite(t) || t < -90 || t > 90)
      throw new FirestoreError(T, "Latitude must be a number between -90 and 90, but was: " + t);
    if (!isFinite(e) || e < -180 || e > 180)
      throw new FirestoreError(T, "Longitude must be a number between -180 and 180, but was: " + e);
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  st(t) {
    if (0 === t.length)
      throw this._t("Document fields must not be empty");
    if (__PRIVATE_isWrite(this.et) && U.test(t))
...
var __PRIVATE_DeleteFieldValueImpl = class ___PRIVATE_DeleteFieldValueImpl extends FieldValue {
  _toFieldTransform(t) {
    if (2 !== t.et)
      throw 1 === t.et ? t._t(`${this._methodName}() can only appear at the top level of your update data`) : t._t(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);
...
function __PRIVATE_parseUpdateVarargs(t, e, r, n, i, s) {
  ...
  if (s.length % 2 != 0)
    throw new FirestoreError(T, `Function ${e}() needs to be called with an even number of arguments that alternate between field names and values.`);
  for (let t2 = 0; t2 < s.length; t2 += 2)
    a.push(__PRIVATE_fieldPathFromArgument$1(e, s[t2])), u.push(s[t2 + 1]);
  const _ = [], c = ObjectValue.empty();
  for (let t2 = a.length - 1; t2 >= 0; --t2)
    if (!__PRIVATE_fieldMaskContains(_, a[t2])) {
      const e2 = a[t2];
      let r2 = u[t2];
...
function __PRIVATE_parseData(t, e) {
  ...
  if (t instanceof FieldValue)
    return function __PRIVATE_parseSentinelFieldValue(t2, e2) {
      if (!__PRIVATE_isWrite(e2.et))
        throw e2._t(`${t2._methodName}() can only be used with update() and set()`);
      if (!e2.path)
        throw e2._t(`${t2._methodName}() is not currently supported inside arrays`);
  ...
  return function __PRIVATE_parseScalarValue(t2, e2) {
    ...
    if ("number" == typeof t2)
      return toNumber(e2.serializer, t2);
    ...
    if (t2 instanceof DocumentReference) {
      ...
      if (!n.isEqual(r))
        throw e2._t(`Document reference is for database ${n.projectId}/${n.database} but should be for database ${r.projectId}/${r.database}`);
      return {
...
function __PRIVATE_fieldPathFromArgument$1(t, e, r) {
  ...
  if ("string" == typeof e)
    return __PRIVATE_fieldPathFromDotSeparatedString(t, e);
...
function query(t, e, ...r) {
  ...
  e instanceof AppliableConstraint && n.push(e), n = n.concat(r), function __PRIVATE_validateQueryConstraintArray(t2) {
    ...
    if (e2 > 1 || e2 > 0 && r2 > 0)
      throw new FirestoreError(T, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(n);
  for (const e2 of n)
    t = e2._apply(t);
  return t;
    ...
    const e = __PRIVATE_newUserDataReader(t.firestore), r = function __PRIVATE_newQueryFilter(t2, e2, r2, n, i, s, o) {
      ...
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s)
          throw new FirestoreError(T, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
          __PRIVATE_validateDisjunctiveFilterElements(o, s);
          const e3 = [];
          for (const r3 of o)
            e3.push(__PRIVATE_parseDocumentIdValue(n, t2, r3));
          a = {
            arrayValue: {
              values: e3
            }
          ...
          a = __PRIVATE_parseDocumentIdValue(n, t2, o);
      } else
        "in" !== s && "not-in" !== s && "array-contains-any" !== s || __PRIVATE_validateDisjunctiveFilterElements(o, s), a = __PRIVATE_parseQueryValue(
          r2,
          e2,
          o,
          /* allowArrays= */
...
var QueryCompositeFilterConstraint = class _QueryCompositeFilterConstraint extends AppliableConstraint {
  ...
  _apply(t) {
    ...
    return 0 === e.getFilters().length ? t : (function __PRIVATE_validateNewFilter(t2, e2) {
      ...
      for (const t3 of n)
        __PRIVATE_validateNewFieldFilter(r, t3), r = __PRIVATE_queryWithAddedFilter(r, t3);
...
var QueryOrderByConstraint = class _QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(t) {
    const e = function __PRIVATE_newQueryOrderBy(t2, e2, r) {
      if (null !== t2.startAt)
        throw new FirestoreError(T, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== t2.endAt)
        throw new FirestoreError(T, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_newQueryBoundFromDocOrFields(t, e, r, n) {
  if (r[0] = getModularInstance(r[0]), r[0] instanceof DocumentSnapshot)
    return function __PRIVATE_newQueryBoundFromDocument(t2, e2, r2, n2, i) {
      if (!n2)
        throw new FirestoreError(P, `Can't use a DocumentSnapshot that doesn't exist for ${r2}().`);
      const s = [];
      for (const r3 of __PRIVATE_queryNormalizedOrderBy(t2))
        if (r3.field.isKeyField())
          s.push(__PRIVATE_refValue(e2, n2.key));
        else {
          const t3 = n2.data.field(r3.field);
          if (__PRIVATE_isServerTimestamp(t3))
            throw new FirestoreError(T, 'Invalid query. You are trying to start or end a query using a document for which the field "' + r3.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
          if (null === t3) {
    ...
    }(t._query, t.firestore._databaseId, e, r[0]._document, n);
  {
    ...
    return function __PRIVATE_newQueryBoundFromFields(t2, e2, r2, n2, i2, s) {
      ...
      if (i2.length > o.length)
        throw new FirestoreError(T, `Too many arguments provided to ${n2}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
      const a = [];
      for (let s2 = 0; s2 < i2.length; s2++) {
        const u = i2[s2];
        if (o[s2].field.isKeyField()) {
          if ("string" != typeof u)
            throw new FirestoreError(T, `Invalid query. Expected a string for document ID in ${n2}(), but got a ${typeof u}`);
          if (!__PRIVATE_isCollectionGroupQuery(t2) && -1 !== u.indexOf("/"))
            throw new FirestoreError(T, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${n2}() must be a plain document ID, but '${u}' contains a slash.`);
          const r3 = t2.path.child(ResourcePath.fromString(u));
          if (!DocumentKey.isDocumentKey(r3))
            throw new FirestoreError(T, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${n2}() must result in a valid document path, but '${r3}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(t, e, r) {
  if ("string" == typeof (r = getModularInstance(r))) {
    if ("" === r)
      throw new FirestoreError(T, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(e) && -1 !== r.indexOf("/"))
      throw new FirestoreError(T, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${r}' contains a '/' character.`);
    const n = e.path.child(ResourcePath.fromString(r));
    if (!DocumentKey.isDocumentKey(n))
      throw new FirestoreError(T, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${n}' is not because it has an odd number of segments (${n.length}).`);
    return __PRIVATE_refValue(t, new DocumentKey(n));
  }
  if (r instanceof DocumentReference)
    return __PRIVATE_refValue(t, r._key);
...
function __PRIVATE_validateDisjunctiveFilterElements(t, e) {
  if (!Array.isArray(t) || 0 === t.length)
    throw new FirestoreError(T, `Invalid Query. A non-empty array is required for '${e.toString()}' filters.`);
}
function __PRIVATE_validateNewFieldFilter(t, e) {
  const r = function __PRIVATE_findOpInsideFilters(t2, e2) {
    for (const r2 of t2)
      for (const t3 of r2.getFlattenedFilters())
...
function __PRIVATE_validateQueryFilterConstraint(t, e) {
  if (!(e instanceof QueryFieldFilterConstraint || e instanceof QueryCompositeFilterConstraint))
    throw new FirestoreError(T, `Function ${t}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
function getDocs(t) {
  (function __PRIVATE_validateHasExplicitOrderByForLimitToLast(t2) {
    if ("L" === t2.limitType && 0 === t2.explicitOrderBy.length)
      throw new FirestoreError(v, "limitToLast() queries require specifying at least one orderBy() clause");
...
function getAggregate(t, e) {
  const r = __PRIVATE_cast(t.firestore, Firestore), n = __PRIVATE_getDatastore(r), i = function __PRIVATE_mapToArray(t2, e2) {
    const r2 = [];
    for (const n2 in t2)
      Object.prototype.hasOwnProperty.call(t2, n2) && r2.push(e2(t2[n2], n2, t2));
    return r2;
...
var WriteBatch = class {
  ...
  _verifyNotCommitted() {
    if (this._committed)
      throw new FirestoreError(w, "A write batch can no longer be used after commit() has been called.");
  }
};
function __PRIVATE_validateReference(t, e) {
  if ((t = getModularInstance(t)).firestore !== e)
    throw new FirestoreError(T, "Provided document reference is from a different Firestore instance.");
  return t;
}
...
var Transaction$1 = class {
  ...
  async lookup(t) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0)
      throw this.lastTransactionError = new FirestoreError(T, "Firestore transactions require all reads to be executed before all writes."), this.lastTransactionError;
...
var __PRIVATE_AsyncQueueImpl = class {
  ...
  enqueueAfterDelay(t, e, r) {
    this.Nt(), // Fast-forward delays for timerIds that have been overriden.
  ...
  get(t) {
    ...
    return this._transaction.lookup([e._key]).then((t2) => {
      ...
      if (n.isFoundDocument())
        return new DocumentSnapshot(this._firestore, r, n.key, n, e.converter);
      if (n.isNoDocument())
        return new DocumentSnapshot(this._firestore, r, e._key, null, e.converter);
      throw fail();
    });
  }
  set(t, e, r) {
...
function runTransaction(t, e, r) {
  ...
  !function __PRIVATE_validateTransactionOptions(t2) {
    if (t2.maxAttempts < 1)
      throw new FirestoreError(T, "Max attempts must be at least 1");
  }(i);
...
!function __PRIVATE_registerFirestore() {
  !function __PRIVATE_setSDKVersion(t) {
    ...
    const n = t.getProvider("app").getImmediate(), i = new Firestore(new __PRIVATE_LiteAuthCredentialsProvider(t.getProvider("auth-internal")), new __PRIVATE_LiteAppCheckTokenProvider(t.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(t2, e2) {
      if (!Object.prototype.hasOwnProperty.apply(t2.options, ["projectId"]))
        throw new FirestoreError(T, '"projectId" not provided in firebase.initializeApp.');
  ...
  registerVersion("firestore-lite", "4.4.2", ""), registerVersion("firestore-lite", "4.4.2", "esm2017");
}();
export {
  AggregateField,
